...
amesiowa.df$dropConditions <- ifelse(amesiowa.df$SubClass!= 020 & amesiowa.df$SubClass != 060 & amesiowa.df$SubClass != 080,'01: Not SFR',
  ...
  ifelse(amesiowa.df$LotArea>200000, '14: Not a Normal Lot Area',
  '99: Eligible Sample')
  )))))))))))));
...
model.1 <- lm(SalePrice ~ FirstFlrSF, data=myeligible.population)
...
qqnorm(model.1$residuals, main = 'QQ Plot First Floor Residuals', col = 'salmon')
qqline(model.1$residuals)

# Make a scatterplot
plot(myeligible.population$FirstFlrSF,model.1$residuals/1000, main = 'First Floor SQFT vs. Sales Price Residuals', 
     col = 'deepskyblue', xlab = 'First Floor SQFT', ylab = 'First Floor SQFT - Sales Price Residuals (000)')
