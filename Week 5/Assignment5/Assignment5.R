...
drop.list <- c('SID','PID','LotConfig','LotFrontage','Heating','HeatingQC','CentralAir', 'LowQualFinSF', 'Exterior1',
               'BsmtFinSF1','BsmtFinSF2','CentralAir','YrSold','MoSold','SaleCondition', 'u',
               'BsmtFullBath','BsmtHalfBath','FullBath','HalfBath', 'FireplaceInd1','FireplaceInd2',
               'OverallQual','OverallCond','PoolArea','LandContour', 'Condition1', 'Condition2', 
               'LandSlope', 'RoofStyle', 'dropConditions', 'MiscVal', 'SaleType', 'Fence', 'PoolQC');
...
par(mfrow = c(1,1))
boxplot(myeligible.population$SalePrice/1000, main = 'Sales Price Distribution', col = 'deepskyblue', ylab = 'Sales Price (000)')
...
avgPricePerSQFT <- aggregate(myeligible.population$SalePrice/myeligible.population$totalsqft, by=list(Neighborhood=myeligible.population$Neighborhood), FUN=mean)
...
model.hood <- lm(myeligible.population$SalePrice/myeligible.population$totalsqft ~ myeligible.population$Neighborhood)

summary(model.hood)

mae.1 <- aggregate(abs(model.hood$residuals), by=list(Neighborhood=myeligible.population$Neighborhood), FUN=mean)
mae.1
...
myeligible.population$hood2 <- ifelse(myeligible.population$avgPricePerSQFTCalc>=70 & myeligible.population$avgPricePerSQFTCalc<=80,1,0);
...
model.best1 <- lm(SalePrice ~ FirstFlrSF + LotArea + TotalBsmtSF + GarageArea + YearBuilt, data=myeligible.population)
...
plot(myeligible.population$TotalBsmtSF,model.best1$residuals/1000, main = 'Sales Price Residuals vs. Total Bsmt Area', 
     col = 'deepskyblue', xlab = 'Total Bsmt SQFT', ylab = 'Total Bsmt - Sales Price Residuals (000)')

plot(myeligible.population$GarageArea,model.best1$residuals/1000, main = 'Sales Price Residuals vs. Garage Area', 
...
#************************************************************************************************************************
model.best2 <- lm(log(SalePrice) ~ FirstFlrSF + LotArea + TotalBsmtSF + GarageArea + YearBuilt, data=myeligible.population)
...
plot(myeligible.population$TotalBsmtSF,model.best2$residuals/1000, main = 'Log Sales Price Residuals vs. Total Bsmt Area', 
     col = 'deepskyblue', xlab = 'Total Bsmt SQFT', ylab = 'Total Bsmt - Log Sales Price Residuals (000)')

plot(myeligible.population$GarageArea,model.best2$residuals/1000, main = 'Log Sales Price Residuals vs. Garage Area', 
...
# Make a scatterplot
scatter.smooth(myeligible.population$FirstFlrSF,myeligible.population$SalePrice/1000, main = 'Sales Price vs. First Floor', 
               col = 'salmon', xlab = 'First Floor SQFT', ylab = 'Sales Price (000)')

scatter.smooth(myeligible.population$LotArea,myeligible.population$SalePrice/1000, main = 'Sales Price vs. Lot Area', 
               col = 'salmon', xlab = 'Lot Area SQFT', ylab = 'Sales Price (000)')

scatter.smooth(myeligible.population$TotalBsmtSF,myeligible.population$SalePrice/1000, main = 'Sales Price vs. Total Bsmt Area', 
               col = 'salmon', xlab = 'Total Bsmt SQFT', ylab = 'Sales Price (000)')

scatter.smooth(myeligible.population$GarageArea,myeligible.population$SalePrice/1000, main = 'Sales Price vs. Garage Area', 
